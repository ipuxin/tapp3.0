...
<body>
	<header class="red back" data-page="/order/orderlist/"><tit>我的夺宝</tit></header>
	<section id="page_team_info">
		<probox>
			<shopname><?=$team['ShopName'];?></shopname>
			<input type="hidden" name="id" value="<?=$team['ProductRealId']?>" id="id">
			<img src="<?=$team['ProductInfo']['ImageMin']?>" class="page" data-page="/product/info/<?=$team['ProductId']?>">
			<tit><?=$team['ProductInfo']['ProductName']?></tit>
			<input type="hidden" name="ProductType" value="<?=$team['ProductInfo']['ProductType']?>">
			<p>
	...
	<?}else{?>
		<footer class="red warp" style="height: 60px;">
			...
			<warp style="padding:3px 16px;">
				<?if($IsInTeam){?>
					<div><i class="icon font page" data-page="/product/info/<?=$team['ProductId']?>">¥<?=$team['ProductInfo']['Prices']['Team']?></i>再次参团</div>
				<?}else{?>
					<div><i class="icon font page" data-page="/product/info/<?=$team['ProductId']?>">¥<?=$team['ProductInfo']['Prices']['Team']?></i>立即参团</div>
				<?}?>
			</warp>
		</footer>
	<?}?>
