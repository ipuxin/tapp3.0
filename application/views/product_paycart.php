...
?>
<style>
probox{margin:0 20px;padding:50px 0 20px 0;display:block;border-bottom:1px solid #ddd;padding-left:110px;position:relative;min-height:100px;}
probox shopname{display:block;position:absolute;left:0;top:15px;font-weight:bolder;font-size:15px;}
probox img{width:100px;height:100px;position:absolute;left:-110px;top:0px;}
...
<body>
	...
	<section id="page_shop">
		...
		<probox>
			...
			<div style="position:relative;" class="productBox" data-cartid="<?=$pro['CartId']?>" data-count="<?=$pro['ProductCount']?>">
			<img src="<?=$pro['ImageMin']?>">
			<tit><?=$pro['ProductName']?><i class="hot"></i></tit>
			<p>
				<price><r>¥<?=$pro['Prices']['Normal']?></r><s><?=$pro['Prices']['Market']?></s></price>
		...
		<orderset>
			...
			<p class="t">配送方式：
				<span><f>快递 <?if($Info['freightAmout']){echo $Info['freightAmout']."元";}else{echo "免邮";}?></f> ></span>
			</p>
			<p class="t">买家留言：</p>
			<p>
		...
		<tongji>
			共<r>1</r>件商品 &nbsp; <b>合计：<r>¥<?=$Info['PricesAll']?></r></b>
		</tongji>
		<payset>
	...
	<script type="text/javascript" src="<?=$staticPath?>js/product_paycart.js?v=<?=$version?>"></script>
