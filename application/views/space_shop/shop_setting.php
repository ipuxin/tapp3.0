...
<body>
	...
	<section id="page_shop_setting">
		...
		<tit>其他</tit>
		<line>
			<t>绑定城市</t><span><g><?=$shop['CityName']?></g>></span>
		</line>
		<line>
			<t>消息设置</t><span>></span>
		</line>
	...
	<script type="text/javascript">
		...
		$('footer btn').click(function(){
			var data = getData();
			if(data){
				ajaxLocal('/space_shop/shopUpd',data,function(json){
					if(json.Num){
						pageTurn('/space_shop');
					}else{
						Msg('店铺修改失败!');
		...
		function getData(){
			...
			if(!ReturnAddress){Msg('请输入退货地址');return;}

			var data = {ShopName:ShopName,ShopOwnerName:ShopOwnerName,ShopOwnerMobile:ShopOwnerMobile,ShopDescription:ShopDescription,ShopAddress:ShopAddress,DeliverAddress:DeliverAddress,ReturnAddress:ReturnAddress};
