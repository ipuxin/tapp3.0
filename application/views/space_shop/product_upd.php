...
<body>
	...
	<section id="page_shop_setting">
		...
		<line class="b" style="margin-bottom:30px;">
			<textarea id="ProductName" placeholder="请输入宝贝标题"><?=$pro['ProductName']?></textarea>
			<fontbox><f><?=$pro['ProductNameCount']?></f>/60</fontbox>
		</line>
		...
		<line class="b in">
			<t>价格</t>
			<input id="Prices_Normal" placeholder="" value="<?=$pro['Prices']['Normal']?>"></input>
			<dw>¥</dw>
		</line>
		<line class="b in">
			<t>库存</t>
			<input id="StorageCount" placeholder="" value="<?=$pro['StorageCount']?>"></input>
		</line>
		<line class="b in">
			<t>运费</t>
			<input id="freightAmout" placeholder="" value="<?=$pro['freightAmout']?>"></input>
			<dw>¥</dw>
		</line>
		<line class="b in">
			<t>发货地</t>
			<input id="DeliverAddress" placeholder="上海市名航区国定东路200号" value="<?=$pro['DeliverAddress']?>"></input>
			<dw>></dw>
		</line>
		<line class="b in">
			<t>描述</t>
			<input id="Description" placeholder="请输入宝贝描述" value="<?=$pro['Description']?>"></input>
			<dw>></dw>
		</line>
		<line class="b">
			<t>上架</t>
			<btn id="IsForSale" class="check <?if($pro['IsForSale']){?>sel<?}?>"><check></check></btn>
		</line>
	</section>
	<footer>
		<btn>确认修改</btn>
	...
	<script type="text/javascript">
		...
		$('footer btn').click(function(){
			var data = getData();
			if(data){
				ajaxLocal('/space_shop/fabuBaobei/upd',data,function(json){
					if(json.Num){
						pageTurn('/space_shop');
					}else{
						Msg('宝贝编辑失败!');
		...
		function getData(){
			...
			var data = {id:ProductId,ProductName:ProductName,Prices:Prices,StorageCount:StorageCount,freightAmout:freightAmout,DeliverAddress:DeliverAddress,Description:Description,IsForSale:IsForSale};
