...
<body>
	...
	<section id="page_shop">
		...
		<orderset>
			<p class="t" style="margin-bottom:20px;">购买数量：
				<span data-kuaidi="<?=$pro['freightAmout']?>" data-price="<?=$pro['Prices']['Normal']?>" style="margin-top:-6px;">
					<btn class="del">-</btn>
					<num>1</num>
					<btn class="add">+</btn>
				</span>
			...
			<p class="t">配送方式：
				<span><f class="kd">快递 <?if($pro['freightAmoutReal']){echo $pro['freightAmoutReal']."元";}else{echo "免邮";}?></f> ></span>
			</p>
			<p class="t">买家留言：</p>
			<p>
		...
		<tongji>
			共<r>1</r>件商品 &nbsp; <b>合计：<r>¥<?=$pro['Prices']['Normal']+$pro['freightAmoutReal']?></r></b>
		</tongji>
		<payset>
	...
	<script type="text/javascript" src="<?=$staticPath?>js/product_payone.js?v=<?=$version?>"></script>
	<script type="text/javascript">
		var ProductId = '<?=$pro["ProductId"]?>';
		var ProductNum = 1;
		var FreightFreeAmout = '<?=$shop["FreightFreeAmout"]?>';
		var FreightAmout = '<?=$pro["freightAmout"]?>';
